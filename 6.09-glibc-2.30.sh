...
make check > $working/6.09-results.txt

# prevent a warning
touch /etc/ld.so.conf

# fix a check that fails in LFS environment
...
# install minimum set of locales for test coverage
...
# execute instead of above to install all locales (time consuming)
...
# install and setup time zone data
...
# to determine local time zone run: tzselect
# and replace <<America/Los_Angeles>>
# create /etc/localtime file by running:
ln -sfv /usr/share/zoneinfo/America/Los_Angeles /etc/localtime

# create new /etc/ld.so.conf
...
#  add directory search capability:
cat >> /etc/ld.so.conf << "EOF"
